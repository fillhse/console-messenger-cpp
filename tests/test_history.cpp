...
TEST_SUITE("history")
{
    ...
    TEST_CASE("order of IDs irrelevant")
    {
        ...
        CHECK(load_history_for_users("123","456") ==
              load_history_for_users("456","123"));
    }
    ...
    TEST_CASE("separate chats don’t mix")
    {
        ...
        CHECK(load_history_for_users("123","456").find("msg_2") ==
              std::string::npos);
        CHECK(load_history_for_users("777","888").find("msg_1") ==
              std::string::npos);
    }
    ...
    TEST_CASE("file created on disk")
    {
        ...
        if(fs::exists("HISTORY"))
            for(auto const& p: fs::directory_iterator("HISTORY")){
                ...
                if(name.find("123")!=std::string::npos &&
                   name.find("456")!=std::string::npos){
                    found=true; break;
                }
            }
        CHECK(found);
